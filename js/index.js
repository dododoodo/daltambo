...
pTags.forEach((pTag) => {
    ...
    pTag.innerHTML = text.repeat(10); // 텍스트를 원하는 만큼 반복하여 길게 만듦
});

function animate() {
    pTags.forEach((pTag) => {
        const distance = pTag.clientWidth; // 텍스트의 가로 너비를 가져옴
        const currentPos = pTag.getBoundingClientRect().left; // 텍스트의 현재 위치를 가져옴

        if (currentPos < -distance) {
            pTag.style.transform = `translateX(0)`; // 텍스트가 왼쪽으로 넘어가면 다시 처음으로 이동시킴
        } else {
            pTag.style.transform = `translateX(-1px)`; // 텍스트를 왼쪽으로 1px씩 이동시킴
        }
    });
